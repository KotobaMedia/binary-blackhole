...
function getDynamoDBCommand() {
  try {
    ...
    const runningContainers = execSync(`docker ps -q -f name=${DDB_CONT}`)
      .toString()
      .trim();
    ...
    const existingContainers = execSync(`docker ps -a -q -f name=${DDB_CONT}`)
      .toString()
      .trim();
...
const { result } = concurrently(
  [
    ...
    {
      command:
        'cargo lambda watch --bin api-streaming --features="streaming" -P 8999',
